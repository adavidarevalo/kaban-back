...
export const createTask = async (
  ...
  try {
    ...
    const newTask = formatNewTask(task)
    ...
    await Board.findOneAndUpdate({ _id: task.boardId }, newBoard)
...
export const deleteTask = async (
  ...
  try {
    ...
    await Board.findOneAndUpdate({ _id: boardId }, boardToUpdate)
...
export const updateTask = async (
  ...
  try {
    ...
    const taskToUpdate = _.omit(task, [
      'previousColumnId',
      'boardId',
      'columnId',
    ])
    ...
    if (task?.previousColumnId) {
      boardToUpdate = removeTaskFromBoard(
        boardToUpdate,
